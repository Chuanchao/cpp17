//
// Created by Chuanchao Lyu on 2023/8/29.
//
...
Config::Config(string configFile) {
    ...
    if (!in) {
        //_logger->error("cannot open input file'{}'", configFile);
        exit(2);
    }
...
string Config::pString(string name) {
    ...
    if (i == symbols.end()) {
        //std::cout << "access of missing property " << name << std::endl;
        //_logger->critical("access of missing property '{}' ({})", name, debugInfo);
        exit(4);
    }
    return i->second;
}
